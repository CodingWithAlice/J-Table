...
隔天重做列表只展示最近 7 天内仍需重做的题目；更早的错题会从列表摘掉，历史做题记录仍保留。
| 当天第一次修改题目答案 | +1 |
| 添加新题目成功 | +1 |
...
公式：`附加 = floor(时长分钟 / 30)`，时长后金币 = 基础金币 + 附加。
...
当天是否「学习过」以该日金币 > 0 为准（做题、改答案、加新题都算）。**今天的倍率只看昨天和前天**，今天自己的入账不参与判定；入账时在「时长后金币」上再乘倍率。
